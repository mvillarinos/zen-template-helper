from datetime import datetime

month_mapping = {
    "es": {
        "Jan": "enero", "Feb": "febrero", "Mar": "marzo",
        "Apr": "abril", "May": "mayo", "Jun": "junio",
        "Jul": "julio", "Aug": "agosto", "Sep": "septiembre",
        "Oct": "octubre", "Nov": "noviembre", "Dec": "diciembre"
    },
    "en": {
        "Jan": "January", "Feb": "February", "Mar": "March",
        "Apr": "April", "May": "May", "Jun": "June",
        "Jul": "July", "Aug": "August", "Sep": "September",
        "Oct": "October", "Nov": "November", "Dec": "December"
    }
}
day_mapping = {
    "1": "1st", "2": "2nd", "3": "3rd", "4": "4th", "5": "5th",
    "6": "6th", "7": "7th", "8": "8th", "9": "9th", "10": "10th",
    "11": "11th", "12": "12th", "13": "13th", "14": "14th", "15": "15th",
    "16": "16th", "17": "17th", "18": "18th", "19": "19th", "20": "20th",
    "21": "21st", "22": "22nd", "23": "23rd", "24": "24th", "25": "25th",
    "26": "26th", "27": "27th", "28": "28th", "29": "29th", "30": "30th",
    "31": "31st"
}

def extract_time(appointment_on):
    return " ".join(appointment_on.split()[-2:]).upper()

def extract_first_name(name):
    if name == None: return name
    return name.split()[0]

def format_names(names, lang):
    names =  [extract_first_name(name) for name in names]
    if len(names) == 1:
        return names[0]
    elif len(names) == 2:
        if lang == "es":
            return f"{names[0]} y {names[1]}"
        else:
            return f"{names[0]} and {names[1]}"
    else:
        if lang == "es":
            return f"{', '.join(names[:-1])} y {names[-1]}"
        else:
            return f"{', '.join(names[:-1])} and {names[-1]}"

def format_plural(count, lang):
    if lang == 'es':
        if count == 2:
            return "ambas"
        if count == 3:
            return "las tres"
        if count == 4:
            return "las cuatro"
        if count == 5:
            return "las cinco"
        if count == 6:
            return "las seis"
        if count == 7:
            return "las siete"
        if count == 8:
            return "las ocho"
    elif lang == 'en':
        if count == 2:
            return "both"
        if count == 3:
            return "the three of you"
        if count == 4:
            return "the four of you"
        if count == 5:
            return "the five of you"
        if count == 6:
            return "the six of you"
        if count == 7:
            return "the seven of you"
        if count == 8:
            return "the eight of you"


# CSV column aliases → canonical field names used internally
CSV_ALIASES = {
    "Customer Name": "name",
    "Type": "template_type",
    "Treatment Name": "service_name",
    "Appointment On": "appointment_on",
    "Group ID": "group_id",
    "Customer Mobile Phone": "mobile_phone",
    "Customer Home Phone": "home_phone",
}

# Required CSV columns (original names) for validation
REQUIRED_CSV_COLUMNS = {"Customer Name", "Type", "Treatment Name"}

# Number of header lines to skip before the CSV header row (none for Appointments CSVs)
HEADER_SKIP_LINES = 0


class AppointmentsParser:
    """
    Represents a parsed appointment client.

    Internally uses canonical field names. CSV inputs are normalized via CSV_ALIASES
    before being passed here.
    """

    def __init__(self, name, template_type, group_id=None, phone=None):
        self.name = name
        self.client_names = [name]
        self.template_type = template_type
        self.group_id = group_id
        self.phone = phone
        self.services = []

    def add_service(self, service_name, appointment_on, client=None):
        self.services.append({
            "service": service_name,
            "time": appointment_on,
            "client": client
        })
        if client is not None and client not in self.client_names:
            self.client_names.append(client)

    def get_first_name(self):
        return extract_first_name(self.name)

    def get_clients_count(self):
        return len(self.client_names)

    def get_formatted_names(self, lang="es"):
        return format_names(self.client_names, lang)

    def get_date(self, lang='es'):
        date_obj = datetime.strptime(self.services[0]['time'], "%b %d, %Y %I:%M %p")
        day = str(int(date_obj.strftime("%d")))
        month = month_mapping[lang][date_obj.strftime("%b")]
        if lang == 'es':
            return f"{day} de {month}"
        else:
            return f"{month} {day_mapping[day]}"

    def get_formatted_services(self, lang="es"):
        selected_services = ""
        if self.template_type == "Standalone":
            selected_services = f"\n🕒 {extract_time(self.services[0]['time'])} - {self.services[0]['service']}"
        elif self.template_type == "Group":
            groups = []
            for service in self.services:
                if service['time'] not in groups:
                    groups.append(service['time'])
            grouped_services = {time: [] for time in groups}
            for service in self.services:
                grouped_services[service['time']].append(service)
            sorted_grouped_services = sorted(grouped_services.items(), key=lambda x: datetime.strptime(x[0], "%b %d, %Y %I:%M %p"))
            for time, services in sorted_grouped_services:
                selected_services += f"\n🕒 {extract_time(time)} - "
                all_same_service = all(s['service'] == services[0]['service'] for s in services)
                if all_same_service and len(services) > 1:
                    if lang == 'es':
                        selected_services += f"{services[0]['service']} para {format_plural(len(services),lang)}"
                    elif lang == 'en':
                        selected_services += f"{services[0]['service']} for {format_plural(len(services),lang)}"
                    if len(services) < self.get_clients_count():
                        selected_services += f" {format_names([s['client'] for s in services], lang)}"
                else:
                    for service in services:
                        if lang == 'es':
                            selected_services += f"{service['service']} para {extract_first_name(service['client'])}, "
                        elif lang == 'en':
                            selected_services += f"{service['service']} for {extract_first_name(service['client'])}, "
                    selected_services = selected_services[:-2]  # Remove the last comma and space
        elif self.template_type == "Linked" or self.template_type == "Package":
            ordered_services = sorted(self.services, key=lambda x: datetime.strptime(x['time'], "%b %d, %Y %I:%M %p"))
            for service in ordered_services:
                selected_services += f"\n🕒 {extract_time(service['time'])} - {service['service']}"
        return selected_services

    def __repr__(self):
        return f"{self.name}, {self.template_type}, services={self.services}"


def parse_csv_rows(rows):
    """
    Parse a list of raw CSV row dicts into AppointmentsParser instances.

    CSV column names are mapped to canonical names via CSV_ALIASES.
    """
    local_clients = []
    for row in rows:
        phone = row.get('Customer Mobile Phone') or row.get('Customer Home Phone') or None
        appointment_type = row['Type']
        if appointment_type == 'Standalone':
            new = AppointmentsParser(name=row['Customer Name'], template_type=row['Type'], phone=phone)
            new.add_service(row['Treatment Name'], row['Appointment On'])
            local_clients.append(new)
        elif appointment_type in ('Linked', 'Package'):
            linked = next((c for c in local_clients if c.name == row['Customer Name']), None)
            if linked:
                linked.add_service(row['Treatment Name'], row['Appointment On'])
            else:
                new = AppointmentsParser(name=row['Customer Name'], template_type=row['Type'], phone=phone)
                new.add_service(row['Treatment Name'], row['Appointment On'])
                local_clients.append(new)
        elif appointment_type == 'Group':
            group_id = row.get('Group ID') or None
            grouped = next((c for c in local_clients if c.group_id and c.group_id == group_id), None) if group_id else None
            if grouped:
                grouped.add_service(row['Treatment Name'], row['Appointment On'], row['Customer Name'])
            else:
                new = AppointmentsParser(name=row['Customer Name'], template_type=row['Type'], group_id=group_id, phone=phone)
                new.add_service(row['Treatment Name'], row['Appointment On'], row['Customer Name'])
                local_clients.append(new)
    return local_clients


def generate_text(client, template, language, operator, location=None):
    """Generate the filled template text for an Appointments client."""
    return template['template'][language].format(
        FirstName=client.get_formatted_names(language),
        Services=client.get_formatted_services(language),
        Date=client.get_date(language),
        Location=location['text'][language] if location else '',
        Operator=operator,
        Plural='s' if language == 'es' and client.get_clients_count() > 1 else ''
    )


def render_ui_groups(app):
    """Render the UI dynamic groups required for the Appointments template type."""
    app.render_operator_group()
    app.render_location_group()


def needs_location():
    return True
